...
<c:if test="${jcr:isNodeType(currentNode, 'bootstrap4mix:advancedNavbar')}">
    ...
    <c:set var="addContainerWithinTheNavbar"
        value="${currentNode.properties.addContainerWithinTheNavbar.boolean}" />
    <c:set var="addLoginButton" value="${currentNode.properties.addLoginButton.boolean}" />
    <c:set var="addLanguageButton"
        value="${currentNode.properties.addLanguageButton.boolean}" />
</c:if>
...
<%-- try to get the expand size --%>
    ...
    <c:if test="${! jcr:isNodeType(curentPageNode,'jmix:navMenuItem')}">
        <c:set var="curentPageNode"
            value="${jcr:getParentOfType(curentPageNode, 'jmix:navMenuItem')}" />
    </c:if>
    ...
    <div data-v-36407026 class="sidebar">
        <div data-v-36407026 data-v-70a4be12 class="tertiary-nav">
            ...
            <template:include view="nacd.base.side" />
