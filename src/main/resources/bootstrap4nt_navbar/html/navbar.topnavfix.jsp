...
<c:if test="${not empty currentNode &&  jcr:isNodeType(currentNode, 'bootstrap4mix:customButtonNavbar')}">
...
<c:if test="${not empty currentNode &&  jcr:isNodeType(currentNode, 'bootstrap4mix:advancedNavbar')}">
...
<c:if test="${not empty curentPageNode &&  !jcr:isNodeType(curentPageNode,'jmix:navMenuItem')}">
...
<nav class="${navClass}">
    <c:if test="${addContainerWithinTheNavbar}">
        <div class="container">asdas
    </c:if>
    <c:choose>
        <c:when test="${not empty rootNode && jcr:isNodeType(rootNode, 'jnt:virtualsite')}">
